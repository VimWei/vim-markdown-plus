...
- 多字节支持的本质是**全链路统一字符单位**，复杂文本操作推荐用 `strcharpart` + `setline`，而不是依赖 Vim 的可视/操作标记。
...
- **charidx({text}, {byte_idx})**
  ...
  - 用法：
    ```vim
    let byte_col = 10
    let char_col = charidx(line, byte_col) + 1  " 得到1-based字符列号
    ```
...
- 只用 `strchars`、`charcol`、`charidx`、`strcharpart`、`getcharpos`、`setcharpos`、`getcursorcharpos`、`setcursorcharpos` 等字符级 API
...
" 构建新内容
let before = strcharpart(line, 0, start_char_col - 1)
let after = strcharpart(line, end_char_col)
...
function! s:ConvertToImageLink()
    ...
    if start_pos[1] == end_pos[1]
        ...
        " 构建新内容
        let before = strcharpart(line, 0, start_char_col - 1)
        let selected = strcharpart(line, start_char_col - 1, end_char_col - start_char_col)
        let after = strcharpart(line, end_char_col)
        ...
        " 处理第一行
        let start_char_col = charidx(first_line, start_pos[2] - 1) + 1
        let first_content = strcharpart(first_line, start_char_col - 1)

        " 处理最后一行
        let end_char_col = charidx(last_line, end_pos[2] - 1) + 1
        let last_content = strcharpart(last_line, 0, end_char_col)
        ...
        " 替换多行
        call setline(start_pos[1], strcharpart(first_line, 0, start_char_col - 1) . new_content)
        if len(lines) > 1
...
**关键转换点说明：**
1. **基数转换**：`getpos()` 返回 1-based 字节列号，用于 `charidx()` 时需要减 1
